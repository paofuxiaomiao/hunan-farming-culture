...
所有AI生成的图片资源备份在 `docs/assets/` 目录中。线上部署使用 Manus 内置 CDN（`/manus-storage/` 路径）。
...
项目通过 Manus 平台部署，域名：`hunancultu-baehqpg8.manus.space`

如需迁移到其他平台：
1. 将 `docs/assets/` 中的图片上传到您的 CDN
2. 全局替换代码中的 `/manus-storage/` 路径为您的 CDN 地址
3. 运行 `pnpm build` 构建静态文件
